...
app.get('/admin', (req, res) => {
  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        body { font-family: 'Helvetica Neue', Arial, sans-serif; background: linear-gradient(135deg, #0f0f0f 0%, #111111 100%); color: #e0e0e0; margin: 0; padding: 3rem 2rem; min-height: 100vh; }
        .container { max-width: 900px; margin: 0 auto; }
        h1 { font-size: 2.5rem; font-weight: 300; letter-spacing: 0.05em; color: #ffffff; margin-bottom: 0.5rem; }
        .subtitle { font-size: 1.1rem; color: #888; margin-bottom: 2.5rem; }
        form { background: rgba(30,30,30,0.6); backdrop-filter: blur(12px); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 2.5rem; margin-bottom: 3rem; }
        label { display: block; font-size: 0.95rem; color: #bbb; margin-bottom: 0.5rem; }
        input { width: 100%; padding: 1rem; background: rgba(40,40,40,0.7); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: #fff; font-size: 1rem; margin-bottom: 1.5rem; transition: border-color 0.3s; }
        input:focus { outline: none; border-color: #ffffff; }
        button { background: #ffffff; color: #000; border: none; padding: 1rem 2rem; font-size: 1.05rem; font-weight: 600; border-radius: 8px; cursor: pointer; transition: background 0.3s, transform 0.2s; }
        button:hover { background: #f0f0f0; transform: translateY(-2px); }
        pre { background: rgba(20,20,20,0.8); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 1.5rem; font-family: 'Courier New', monospace; white-space: pre-wrap; overflow: auto; }
        a { color: #ffffff; text-decoration: none; opacity: 0.7; }
        a:hover { opacity: 1; }
      </style>
    </head>
    <body>
      <div class="container">
        ...
        <form action="/admin/add?pass=${ADMIN_PASSWORD}" method="POST">
          ...
          <label>Themes</label>
          <input type="text" name="themes" readonly value="Aether Starter (Shopify Theme)" style="background:#222; color:#888; cursor:default;">
        ...
        <br><a href="/admin?pass=${ADMIN_PASSWORD}">Refresh</a>
      </div>
    </body>
    </html>
  `);
...
app.post('/admin/add', (req, res) => {
  ...
  const { domain, exp } = req.body;
  if (!domain || !exp) return res.send('<h1>Error</h1><p>All fields required.</p><a href="/admin?pass=' + ADMIN_PASSWORD + '">Back</a>');

  const normalizedDomain = domain.trim().toLowerCase();
  const key = generateKey();
  const themes = ['Aether Starter (Shopify Theme)'];

  LICENSES[normalizedDomain] = { key, exp, themes };
  ...
  try {
    ...
    res.send(`
      ...
      <style>body{font-family:'Helvetica Neue',Arial,sans-serif;background:#111;color:#e0e0e0;padding:4rem 2rem;text-align:center;}
      h1{font-size:2.2rem;font-weight:300;color:#ffffff;} p{margin:1rem 0;font-size:1.1rem;}
      ...
      <body>
        ...
        <p><strong>Theme:</strong> Aether Starter (Shopify Theme)</p>
        <br><a href="/admin?pass=${ADMIN_PASSWORD}">Back to Admin</a>
      </body>
      </html>
    `);
  } catch (err) {
